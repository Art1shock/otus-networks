...
Назначенным будет порт на том коммутаторе, где наименьшая стоимость пути до корневого моста. Видно, что стоимость путей одинакова, поэтмоу и сравниваем по BID (он включает в себя приоритет и MAC-адрес моста), но так как приоритеты не менялись (они везде одинаковые), то сравнивается по MAC-адресу.
