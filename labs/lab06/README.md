...
Если напротив корневых портов находятся назначенные порты, то с назначенными и альтернативными портами не все так однозначно. Назначенным портом будет порт на том коммутаторе, чей MAC-адрес будет иметь самое низкое значение (при условии, что приоритет одинаковый, так как он еще не выставлен вручную). Соответственно на противоположной стороне порт станет альтернативным и канал передачи становится заблокирован. В моем случае рассмотрим канал между коммутаторами S1 (Fa0/4) и S3 (Fa0/4). У коммутатора S3 MAC-адрес имеет более низкое значение, значит порт Fa0/4 на этом коммутаторе по логике будет назначенным, а на другой стороне (на S1) порт станет альтернативным, и если взглянуть на изображение выше, то так оно и есть.
