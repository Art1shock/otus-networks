...
![](https://github.com/Art1shock/images/blob/main/%D0%A1%D0%BA%D1%80%D0%B8%D0%BD%D1%8B%20%D0%B4%D0%BB%D1%8F%20%D0%94%D0%97_7/Screenshot_9.png)

#### В схему ниже запишите роль и состояние (Sts) активных портов на каждом коммутаторе в топологии:

![](https://github.com/Art1shock/images/blob/main/%D0%A1%D0%BA%D1%80%D0%B8%D0%BD%D1%8B%20%D0%B4%D0%BB%D1%8F%20%D0%94%D0%97_7/Screenshot_10.1.png)
...
Корневые порты есть на коммутаторах S1 (Fa0/1) и S3 (Fa0/1).
...
Альтернативные порты находятся на коммутаторах S1 (Fa0/2, Fa0/3, Fa0/4) и S3 (Fa0/2).
...
Если напротив корневых портов находятся назначенные порты, то с назначенными и альтернативными портами не все так однозначно. Назначенным портом будет порт на том коммутаторе, чей BID будет иметь самое низкое значение (при условии, что приоритет одинаковый, выбирается самый низкий по значению MAC-адрес) или можно сказать так - альтернативным портом будет порт с самым высоким BID. Соответственно на противоположной стороне порт станет альтернативным и канал передачи становится заблокирован. В моем случае рассмотрим канал между коммутаторами S1 (Fa0/4) и S3 (Fa0/4). У коммутатора S3 MAC-адрес имеет более низкое значение, значит порт Fa0/4 на этом коммутаторе по логике будет назначенным, а на другой стороне (на S1) порт станет альтернативным, и если взглянуть на изображение выше, то так оно и есть.
...
Возьмем для примера заблокированный порт у коммутатора S1 (Fa0/4)
